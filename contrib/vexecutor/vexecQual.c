		...
		case T_Aggref:
			{
				...
				if (parent && IsA(parent, AggState))
				{
					...
					astate->args = (List *) VExecInitExpr((Expr *) astate->inputTargets,
														 parent);
		...
		case T_WindowRef:
			{
				...
				wrstate->args = (List *) VExecInitExpr((Expr *) windowref->args,
													  parent);
				/* 
		...
		case T_ArrayRef:
			{
				...
				astate->refupperindexpr = (List *)
					VExecInitExpr((Expr *) aref->refupperindexpr, parent);
				astate->reflowerindexpr = (List *)
					VExecInitExpr((Expr *) aref->reflowerindexpr, parent);
				astate->refexpr = VExecInitExpr(aref->refexpr, parent);
				astate->refassgnexpr = VExecInitExpr(aref->refassgnexpr,
													parent);
		...
		case T_FuncExpr:
			{
				...
				fstate->args = (List *)
					VExecInitExpr((Expr *) funcexpr->args, parent);
		...
		case T_DistinctExpr:
			{
				...
				fstate->args = (List *)
					VExecInitExpr((Expr *) distinctexpr->args, parent);
				fstate->func.fn_oid = InvalidOid;		/* not initialized */
				state = (ExprState *) fstate;
			}
			break;
		...
		case T_BoolExpr:
			{
				...
				bstate->args = (List *)
					VExecInitExpr((Expr *) boolexpr->args, parent);
				state = (ExprState *) bstate;
			}
			break;
		case T_SubPlan:
			{
				/* Keep this in sync with VExecInitExprInitPlan, below */
				...
				sstate->testexpr =
					VExecInitExpr((Expr *) subplan->testexpr, parent);
				sstate->args = (List *)
					VExecInitExpr((Expr *) subplan->args, parent);
		...
		case T_FieldSelect:
			{
				...
				fstate->arg = VExecInitExpr(fselect->arg, parent);
				fstate->argdesc = NULL;
				state = (ExprState *) fstate;
			}
			break;
		case T_FieldStore:
			{
				...
				fstate->arg = VExecInitExpr(fstore->arg, parent);
				fstate->newvals = (List *) VExecInitExpr((Expr *) fstore->newvals, parent);
				fstate->argdesc = NULL;
				state = (ExprState *) fstate;
			}
			break;
		case T_RelabelType:
			{
				...
				gstate->arg = VExecInitExpr(relabel->arg, parent);
				state = (ExprState *) gstate;
			}
			break;
		case T_ConvertRowtypeExpr:
			{
				...
				cstate->arg = VExecInitExpr(convert->arg, parent);
		...
		case T_CaseExpr:
			{
				...
				cstate->arg = VExecInitExpr(caseexpr->arg, parent);
				foreach(l, caseexpr->args)
				{
					...
					wstate->expr = VExecInitExpr(when->expr, parent);
					wstate->result = VExecInitExpr(when->result, parent);
					outlist = lappend(outlist, wstate);
				}
				cstate->args = outlist;
				cstate->defresult = VExecInitExpr(caseexpr->defresult, parent);
				state = (ExprState *) cstate;
			}
			break;
		case T_ArrayExpr:
		...
		case T_NullIfExpr:
			{
				...
				fstate->args = (List *)
					VExecInitExpr((Expr *) nullifexpr->args, parent);
				fstate->func.fn_oid = InvalidOid;		/* not initialized */
				state = (ExprState *) fstate;
			}
			break;
		case T_NullTest:
			{
				...
				nstate->arg = VExecInitExpr(ntest->arg, parent);
		...
		case T_BooleanTest:
			{
				...
				gstate->arg = VExecInitExpr(btest->arg, parent);
				state = (ExprState *) gstate;
			}
			break;
		case T_CoerceToDomain:
			{
				...
				cstate->arg = VExecInitExpr(ctest->arg, parent);
		...
		case T_PercentileExpr:
			{
				...
				pstate->args = (List *) VExecInitExpr((Expr *) pstate->tlist, parent);
		...
		case T_TargetEntry:
			{
				...
				gstate->arg = VExecInitExpr(tle->expr, parent);
				state = (ExprState *) gstate;
			}
			break;
		case T_List:
